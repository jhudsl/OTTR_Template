...
output_directory <- file.path("docs", "coursera")
...
old_files <- list.files(output_directory, pattern = c("html$", "md$"), full.names = TRUE)
...
lapply(needed_directories, function(needed_dir) {
  if (!dir.exists(file.path(output_directory, needed_dir))) {
    fs::dir_copy(needed_dir, file.path(output_directory, needed_dir), overwrite = TRUE)
  }
})

# Slightly different path for the libs folder
libs_path <- file.path("docs", "libs")
if (!dir.exists(file.path(output_directory, "libs"))) {
  fs::dir_copy(libs_path, file.path(output_directory, "libs"), overwrite = TRUE)
}
...
yaml::write_yaml(output_yaml, file.path(output_directory, "_output_coursera.yml"))


message("render bookdown")
# Do the render
bookdown::render_book(
  input = "index.Rmd",
  output_yaml = file.path(output_directory, "_output_coursera.yaml"),
  output_dir = file.path("docs", "coursera"))

# Read in TOC closing CSS lines
toc_close_css <- readLines(output_yaml$`bookdown::gitbook`$css)
full_css <- readLines(file.path(output_directory, "assets", "style.css"))

# Write to "style.css"
writeLines(append(full_css, toc_close_css), file.path(output_directory, "assets", "style.css"))
